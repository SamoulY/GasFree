...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('pharos_faucet_server.log')
    ]
)
...
CONTRACT_ADDRESS = os.getenv('CONTRACT_ADDRESS', '0x171026a74CE726CD802fFC2e519036c22aa7F56b')
...
RELAYER_ADDRESS = os.getenv('RELAYER_ADDRESS', RELAY_SIGNER_ADDRESS)
AD_TOKEN = "demo_ad_ok"
CHAIN_ID = 688689
...
# ==================== 【核心修复】全局连接状态变量 ====================
# 我们将在此处初始化 Web3 并立即检查连接状态，避免后续API中的检查错误
PHAROS_RPC_URL = "https://atlantic.dplabs-internal.com"
w3_provider = Web3(Web3.HTTPProvider(PHAROS_RPC_URL))
...
try:
    if w3_provider.is_connected():
        NETWORK_CHAIN_ID = w3_provider.eth.chain_id
        RPC_CONNECTED = True
        logger.info(f"✅ Connected to PHAROS Atlantic Testnet (Chain ID: {NETWORK_CHAIN_ID})")
        if NETWORK_CHAIN_ID != CHAIN_ID:
            logger.warning(f"⚠️  Configured Chain ID ({CHAIN_ID}) differs from network Chain ID ({NETWORK_CHAIN_ID})")
    else:
        logger.error("❌ Failed to connect to PHAROS Atlantic Testnet")
...
def init_db():
    try:
        conn = sqlite3.connect('pharos_faucet.db', check_same_thread=False)
        c = conn.cursor()
        c.execute('''
...
def get_connection():
    return sqlite3.connect('pharos_faucet.db', check_same_thread=False)

@app.route('/')
def home():
    return jsonify({
        "name": "PHAROS Atlantic Testnet Faucet API",
        "version": "1.0",
        "status": "running",
        "contract": CONTRACT_ADDRESS,
        "signer": RELAY_SIGNER_ADDRESS,
        "relayer": RELAYER_ADDRESS,
        "contract_balance_url": f"https://atlantic.pharosscan.xyz/address/{CONTRACT_ADDRESS}"
    })
...
def health_check():
    ...
    try:
        if RPC_CONNECTED:
            ...
            contract_balance_phrs = w3_provider.from_wei(contract_balance, 'ether')
    except Exception as e:
    ...
    return jsonify({
        ...
        "chain_id": CHAIN_ID,
        "contract_balance": str(contract_balance_phrs) + " PHRS",
...
def verify_ad():
    ...
    try:
        ...
        # 验证地址格式
        try:
            wallet_address = w3_provider.to_checksum_address(wallet_address)
        except:
            # 手动验证
        ...
        message_hash = w3_provider.solidity_keccak(
            ['address', 'uint256', 'uint256'],
            [w3_provider.to_checksum_address(wallet_address), nonce, deadline]
        )
    ...
    try:
        ...
        try:
            ...
            return jsonify({
                "success": True,
                "tx_hash": tx_hash_hex,
                "explorer_url": f"https://atlantic.pharosscan.xyz/tx/{tx_hash_hex}",
    ...
    try:
        ...
        try:
            if RPC_CONNECTED:
                ...
                # 计算大约可以领取多少次 (0.01 PHRS = 0.01 * 10^18 wei)
        ...
        return jsonify({
            ...
            "explorer_url": f"https://atlantic.pharosscan.xyz/address/{CONTRACT_ADDRESS}",
            "contract_balance": str(contract_balance_phrs) + " PHRS",
            "approx_remaining_claims": approx_claims,
            "claim_amount": "0.01 PHRS",
            "note": "This is a relay faucet - users pay no gas. Server pays gas fees.",
            "server_status": "online",
            "rpc_connected": RPC_CONNECTED  # 【修复点1】使用全局变量
        })
    except Exception as e:
...
def server_status():
    ...
    try:
        ...
        return jsonify({
            ...
            "rpc_connection": "connected" if RPC_CONNECTED else "disconnected",  # 【修复点2】使用全局变量
            "contract_address": CONTRACT_ADDRESS,
            "signer_address": RELAY_SIGNER_ADDRESS,
            "relayer_address": RELAYER_ADDRESS
        })
...
if __name__ == '__main__':
    ...
    print("=" * 60)
    print("🚀 PHAROS ATLANTIC TESTNET RELAY FAUCET SERVER")
    print("=" * 60)
    print(f"📡 Server: http://{host}:{port}")
    print(f"🔗 Contract: {CONTRACT_ADDRESS}")
    print(f"✍️  Signer: {RELAY_SIGNER_ADDRESS}")
    print(f"🚀 Relayer: {RELAYER_ADDRESS}")
    print(f"🌐 Network: PHAROS Atlantic Testnet (Chain ID: {CHAIN_ID})")
    print(f"💰 Amount: 0.01 PHRS per claim (GAS-FREE)")
    print("=" * 60)
    print("⚠️  IMPORTANT: Check if contract has enough PHRS!")
    print(f"   Explorer: https://atlantic.pharosscan.xyz/address/{CONTRACT_ADDRESS}")
    print("=" * 60)
    print("📋 Endpoints:")
    print("  GET  /                   - API info")
